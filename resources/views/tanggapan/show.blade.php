    ...
    $(function(){
        $(document).ready(function(){
            $('#add_btn').on('click', function(){
                var html='';
                html+='<div id="dynamic_field_tanggapan"><table class="table table-bordered my-3"><thead><tr><th>Tanggapan dari Bidang : @if(Auth::user()->role == "Admin")<br></br><select name="gt_bidangs"><option>Select Bidang</option>@foreach($bidangs as $bidang)<option value="{{$bidang->Bidang->name}}">{{$bidang->Bidang->name}}</option>@endforeach</select>@endif</th></tr></thead><tbody><tr><td><textarea class="form-control" rows="3" name="gt_bidang"></textarea></td></tr></tbody></table></div><div id="dynamic_field_nama"><table class="table table-bordered"><thead><tr><th scope="col">Nama :</th><th scope="col">Tanggal (Bulan/Tanggal/Tahun) :</th></tr><tbody><tr><td><input class="form-control" type="text" name="gt_nama"></td><td><input class="form-control" type="date" value="{{ date("Y-m-d") }}" name="gt_date"></td></tr></tbody></table></div><div class="container mt-3 text-right"><button type="button" class="btn btn-danger mx-2" id="remove_btn"><i class="fa fa-minus"></i></button></div>';
