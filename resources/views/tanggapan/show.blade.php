...
<div class="main">
    ...
    @csrf
        <div class="container my-4, row" id="dynamic_field_append">
            <table class="table table-bordered my-3">
                <thead>
                    <tr>
                        <th>
            ...
            <div id="newRow"></div>
            <button type="button" class="btn btn-success mt-2" id="add_btn"><i class="fa fa-plus"></i> Add more</button>
...
<script>
    ...
    $("#add_btn").click(function () {
        var html = '';
        html+='<div id="dynamic_field_append"><table class="table table-bordered my-3"><thead><tr><th>A. Tanggapan dari Bidang :@if(Auth::user()->role == 'Admin')<br><br><select name="tg_bidangs[]"><option>Select Bidang</option>@foreach($bidangs as $bidang)<option value="{{$bidang->Bidang->name}}">{{$bidang->Bidang->name}}</option>@endforeach</select>@endif</th></tr></thead><tbody><tr><td><textarea class="form-control" rows="3" name="tg_bidang[]"></textarea></td></tr></tbody></table><table class="table table-bordered"><thead><tr><th scope="col">Nama :</th><th scope="col">Tanggal (Bulan/Tanggal/Tahun) :</th></tr></thead><tbody><tr><td><input class="form-control" type="text" name="tg_nama[]"></td><td><input class="form-control" type="date" value="{{ date('Y-m-d') }}" name="tg_date[]"></td></tr></tbody></table><button type="button" class="btn btn-danger" id="remove_btn"><i class="fa fa-minus"></i>  Delete</button></div><div class="container text-right mx-2"></div>';
